...
const maintenanceStages = [
  { name: 'MONITORING RUTIN', icon: AlertTriangle, bg: 'bg-gold' },
  { name: 'BUG FIX & UPDATE', icon: Wrench, bg: 'bg-crimson text-paper' },
  { name: 'LAPORAN BERKALA', icon: ChartColumn, bg: 'bg-magenta' },
  { name: 'KEAMANAN & OPTIMASI', icon: ShieldCheck, bg: 'bg-gold' },
] as const
...
export default async function MaintenanceSupportPage() {
  const t = await getTranslations('Pages.MaintenanceSupport')
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 md:gap-4">
          {maintenanceStages.map((stage, index) => (
            <div key={stage.name} className="flex items-center">
              ...
              {index < maintenanceStages.length - 1 ? (
