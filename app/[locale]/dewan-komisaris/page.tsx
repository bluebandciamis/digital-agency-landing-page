...
const commissioners = [
  {
    ...
    bio: 'A. Zhafir Mahfudz Ubaidillah',
  },
] as const

export default async function DewanKomisarisPage() {
  const t = await getTranslations('Pages.DewanKomisaris')
    ...
    <PageLayout label={t('label')} title={t('title')} intro={t('intro')}>
      <article className="rounded-none border-[3px] border-ink bg-paper p-6 shadow-hard-sm">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {commissioners.map(({ name, role, bio }) => (
            <div key={name} className="flex flex-col gap-4 border-[3px] border-ink bg-paper p-4 shadow-hard-sm">
              <div className="flex size-20 items-center justify-center border-[3px] border-ink bg-magenta shadow-hard-sm">
