  ...
  if (state.status === 'success') {
    return (
      <div role="status" className="flex flex-col items-start gap-4 border-[4px] border-ink bg-gold p-8 shadow-hard">
        ...
        <h3 className="font-display text-2xl uppercase">Pesan terkirim!</h3>
    ...
    <form action={action} noValidate className="flex flex-col gap-5 border-[4px] border-ink bg-paper p-6 shadow-hard sm:p-8">
      <div className="grid gap-5 sm:grid-cols-2">
        <Field label="Nama" name="name" error={state.errors?.name}>
          <input id="name" name="name" type="text" autoComplete="name" placeholder="Nama Anda" className={fieldClass} aria-invalid={!!state.errors?.name} />
        </Field>
        <Field label="Email" name="email" error={state.errors?.email}>
          <input id="email" name="email" type="email" autoComplete="email" placeholder="nama@perusahaan.com" className={fieldClass} aria-invalid={!!state.errors?.email} />
        </Field>
      </div>

      <Field label="Layanan" name="service" error={state.errors?.service}>
        <select id="service" name="service" defaultValue="" className={cn(fieldClass, 'appearance-none')} aria-invalid={!!state.errors?.service}>
          ...
          <option value="bundle">Desain + Development (bundle)</option>
        </select>
      </Field>

      <Field label="Ceritakan proyek Anda" name="message" error={state.errors?.message}>
        <textarea id="message" name="message" rows={5} placeholder="Produk apa yang ingin dibuat? Siapa penggunanya? Target rilis kapan?" className={cn(fieldClass, 'resize-y')} aria-invalid={!!state.errors?.message} />
      </Field>
